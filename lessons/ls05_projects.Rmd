...
# Import data
ebola_sierra_leone <- read_csv(here("ch01_getting_started/data_prep/ebola_sierra_leone.csv"))
```
...
In the last lesson, you walked through some of the essential steps of data analysis, from importing data to calculating basic statistics. But you skipped over one crucial step: setting up a data analysis *project*.
...
For RStudio cloud users, go to your Cloud homepage, [rstudio.cloud](https://rstudio.cloud/), and click on the "New Project" button.

![](images/new_project_rstudio_cloud.png){width="447"}

Name your Project `ebola_analysis`:

![](images/name_project_rstudio_cloud.png){width="453"}

The RStudio Project you have now created is just a folder on a virtual computer that has a .Rproj file within it (and maybe a .RHistory file). You should be able to see this .Rproj file in the Files pane of RStudio:
...
If you are on a local computer, open RStudio, then on the RStudio menu, go to `File > New Project`.
...
Call your Project "ebola_analysis" and make it a sub-directory of a folder that is easy to find (e.g. your desktop).
...
Also note that double-clicking on this .RProj file from your computer's Finder/File Explorer will re-open your RStudio project:

![](images/windows_open_project.png){width=30%} ![](images/mac_open_project.png){width=40%}
:::

Note also that there is a header at the top right of RStudio window that tells you which Project you currently have open. Clicking on this gives you some additional Project options.
...
-   **data:** This contains the source (raw) data files that you will use in the analysis. These could be CSV or Excel files, for example. To ensure reproducibility, you should generally avoid using R to edit these files directly; edited files should go to the *outputs* folder (below).
...
district_tab <- tabyl(district_vec)
district_tab
...
RStudio Projects, and the relative paths they allow you to use, are important for reproducibility. Projects that use relative paths can be run on anyone's computer, and the importing and exporting code should work without any hiccups.
...
<!-- TODO: more then export on rstudio cloud -->

If you instead want to view the CSV in Excel, you can navigate to the same file in your computer's Finder/File Explorer and double-click on it from there.
...
# Cases by district ----
district_vec <- ebola_sierra_leone$district
district_tab <- arrange(tabyl(district_vec), -n) # sort the table
district_tab
```

( `-n` means "sort in descending order of the `n` variable")
...
# Cases by district ----
district_vec <- ebola_sierra_leone$district
district_tab <- arrange(tabyl(district_vec), -n)
district_tab
```
...
This command uses the `ggsave()` function to export the `categ_vars_plot` figure. The `plot` argument of `ggsave()` takes in the object to be saved (in this case `categ_vars_plot`), and the `filename` argument will take in the target file path for the plot.
...
To see this newly-saved plot, navigate to it the Files tab. You can click on it to open it with your computer's default image viewer:
