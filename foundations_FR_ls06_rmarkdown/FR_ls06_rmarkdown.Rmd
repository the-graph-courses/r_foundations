...
Avec la sortie définie sur "powerpoint_document", cela donne ceci :
...
Il est également possible d'écrire des blocs de code intégrés dans le texte. Si vous allez en mode *Source* et que vous tapez :

"La somme d'une paire de 2 est \`r 2+2\`"

et que vous compilez le RMD, le document résultant évaluera le code R entre lesbarres obliques. Notez que vous devez inclure le "r" au début de votre bloc de code entre les lignes pour qu'il le reconnaisse comme du code R.
...
Le site de documentation de l'extension propose [une galerie](http://rmarkdown.rstudio.com/gallery.html) des différentes sorties possibles. Vous pouvez créer des diapositives, des sites web ou même des livres entiers, comme ce document.
